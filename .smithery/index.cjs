...
var FreepikClient = class {
  ...
  async getResourceDetails(id) {
    return this.request(`/v1/resources/${id}`);
  }
  async downloadResource(id) {
    return this.request(`/v1/resources/${id}/download`);
  }
  // ============================================
  // Mystic AI (Legacy)
  // ============================================
  async generateImage(params) {
    return this.request("/v1/ai/mystic", {
      method: "POST",
      body: JSON.stringify(params)
    });
  }
  async checkMysticStatus(taskId) {
    return this.request(`/v1/ai/mystic/${taskId}`);
  }
  // ============================================
  // Seedream 4 - Text to Image
  // ============================================
  async seedreamTextToImage(params) {
    return this.request("/v1/ai/text-to-image/seedream-v4", {
      method: "POST",
      body: JSON.stringify(params)
    });
  }
  async checkSeedreamStatus(taskId) {
    return this.request(
  ...
  async checkSeedreamEditStatus(taskId) {
    return this.request(
  ...
  // ============================================
  async waitForCompletion(taskId, type = "seedream", maxAttempts = 60, intervalMs = 2e3) {
...
var configSchema = external_exports.object({
  freepikApiKey: external_exports.string().min(1).describe("Your Freepik API Key (get it from freepik.com/api)")
});
function createServer({ config }) {
  ...
  server.tool(
    ...
    "Generate an image using Seedream 4.0 (ByteDance) - High quality 4K capable",
    {
      ...
      guidance_scale: external_exports.number().min(1).max(10).optional().default(2.5).describe("How closely to follow the prompt (1-10, default 2.5)"),
      wait_for_result: external_exports.boolean().optional().default(true).describe("Wait for image generation to complete")
    },
                ...
                text: JSON.stringify(
                  {
                    ...
                    message: "Generation started. Use seedream_status to check progress."
                  },
                  null,
                  2
                )
...
URL: ${completed.generated[0].url}`
  ...
  server.tool(
    ...
    {
      prompt: external_exports.string().describe("Instructions for editing the image"),
      image_url: external_exports.string().url().describe("URL of the image to edit"),
      guidance_scale: external_exports.number().min(1).max(10).optional().default(2.5).describe("How closely to follow the prompt"),
      wait_for_result: external_exports.boolean().optional().default(true).describe("Wait for edit to complete")
    },
                ...
                text: JSON.stringify(
                  {
                    task_id: result.task_id,
                    status: result.status,
                    message: "Edit started. Use seedream_status to check progress."
                  },
                  null,
                  2
                )
...
URL: ${completed.generated[0].url}`
  ...
  server.tool(
    ...
    {
      task_id: external_exports.string().describe("Task ID to check"),
      type: external_exports.enum(["text-to-image", "edit"]).optional().default("text-to-image").describe("Type of task")
    },
    async ({ task_id, type }) => {
      try {
        const result = type === "edit" ? await client.checkSeedreamEditStatus(task_id) : await client.checkSeedreamStatus(task_id);
...
Image URL: ${result.generated[0].url}`
  ...
  server.tool(
    ...
    {
      term: external_exports.string().describe("Search term"),
      limit: external_exports.number().min(1).max(200).optional().default(20).describe("Number of results (max 200)"),
      order: external_exports.enum(["relevance", "recent"]).optional().default("relevance").describe("Sort order"),
      content_type: external_exports.enum(["photo", "vector", "psd", "all"]).optional().default("all").describe("Filter by content type")
    },
    async ({ term, limit, order, content_type }) => {
      try {
        const filters = content_type !== "all" ? {
          content_type: {
...
${JSON.stringify(summary, null, 2)}`
  ...
  server.tool(
    ...
    {
      id: external_exports.number().describe("Resource ID")
    },
    async ({ id }) => {
      try {
  ...
  server.tool(
    ...
    {
      id: external_exports.number().describe("Resource ID to download")
    },
    async ({ id }) => {
      try {
  ...
  server.tool(
    ...
    {
      prompt: external_exports.string().describe("Text description of the image"),
      resolution: external_exports.enum(["2k", "4k"]).optional().default("2k").describe("Image resolution"),
      aspect_ratio: external_exports.enum([
        "square_1_1",
        "classic_4_3",
        "traditional_3_4",
        "widescreen_16_9",
        "social_story_9_16"
      ]).optional().default("square_1_1").describe("Aspect ratio"),
      realism: external_exports.boolean().optional().default(false).describe("Enable realistic style"),
      wait_for_result: external_exports.boolean().optional().default(true).describe("Wait for completion")
    },
